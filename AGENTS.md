...
- A valid `SimulationResult` supplies the prompt facts and Python-generated Russian analytical candidates about district changes, measure contributions, synergies, clipping, and remaining risks. User-facing explanation and alert text uses full Russian indicator names; abbreviations remain in the technical indicator glossary only. Live candidates are distinct from the deterministic offline wording. The model selects one sentence per briefing field; Python rejects any text outside those candidates after Pydantic validation and assigns `Debrief.source`. This prevents invented facts from being displayed while making live explanations more specific than the offline summary. Missing keys, mock mode, refusal, incomplete output, malformed JSON, unsupported model, timeout, rate limit, and network failure lead to a cached offline briefing.
